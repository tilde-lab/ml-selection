...
def get_structures_and_seebeck(
    ...
    if not raw_str_path:
        ...
        structures_dfrm_mpds = handler.to_order_disordered_str(
            phases=phases, is_uniq_phase_id=is_uniq_structure_for_phase
        )
        file_path = path_to_save + "rep_structures.json"
        structures_dfrm_mpds.write_json(file_path)
    else:
        structures_dfrm_mpds = pl.read_json(raw_str_path)

    result_dfrm = handler.add_seebeck_by_phase_id(median_seebeck, structures_dfrm_mpds)
