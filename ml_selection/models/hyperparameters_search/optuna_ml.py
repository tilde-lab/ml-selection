...
def run_tune_linear_regression(X_train, y_train, X_test, y_test, n_trials=1):
    def objective(trial) -> int:
        ...
        if r2 > BEST_R2:
            ...
            onx = convert_sklearn(
                model,
                initial_types=[("input", FloatTensorType([None, 101]))],
                target_opset=10,
            )
            onnx.save(onx, f"lr_cond_250.onnx")
...
def run_tune_boosted_trees(X_train, y_train, X_test, y_test, n_trials=1):
    def objective(trial) -> int:
        ...
        if r2 > BEST_R2:
            ...
            onnx.save(onx, f"gb_cond_250.onnx")
...
def run_tune_decision_tree(X_train, y_train, X_test, y_test, n_trials=1, num=250):
    def objective(trial) -> int:
        ...
        min_impurity_decrease = trial.suggest_float(
            "min_impurity_decrease", 0.0001, 1.0
        )
        ...
        if r2 > BEST_R2:
            ...
            onnx.save(onx, f"dt_cond_250.onnx")
